...
export class CheckersGame {
  ...
  switchPlayers() {
    ...
    const targetAnim =
      this.graph.camAnimations[this.cameras[this.currentPlayer]];
    midAnim.start(this.graph.scene.animTime);
    targetAnim.start(
      this.graph.scene.animTime + midAnim.duration + this.animTime
    );
    this.currBoard = undefined;
    this.changedObjects = [];
  ...
  init(gamePickables) {
    ...
    for (const pickable of Object.values(gamePickables)) {
      ...
      if (
        ...
        (match[1] >= 21 && match[1] <= 32)
      ) {
        this.graph.components[id].children = this.graph.components[
          id
        ].children.filter((c) => c.id.slice(-7) !== "checker");
        this.graph.components[id].children = this.graph.components[
          id
        ].children.filter((c) => c.id.slice(-5) !== "queen");
        this.graph.components[id].children.push({
          id: id + "_checker",
          type: "component",
        });
  ...
  getAvailableMoves(checker) {
    ...
    for (const move of moves) {
      ...
      if (!pos) continue;
      if (!pos.isChecker()) possibleMoves.push(pos);
      else if (pos.color !== checker.color) {
        const jump = this.getTile(pos.row + move[0], pos.column + move[1]);
        if (jump && !jump.isChecker()) jumpingMoves.push(jump);
      }
    }
    return jumpingMoves.length == 0
      ? { moves: possibleMoves, type: "move" }
      : { moves: jumpingMoves, type: "jump" };
  }
  ...
  getLerp(start, end, p, ease) {
    ...
    const y =
      start[1] +
      (end[1] - start[1]) *
        (ease == "in" ? this.easeInCubic(p) : this.easeOutCubic(p));
    ...
    keyframes.push(
      new MyKeyframe(
        ...
        mat4.translate(
          mat4.create(),
          mat4.create(),
          vec3.fromValues(0, -0.0025, 0)
        )
      )
    );
    keyframes.push(new MyKeyframe(currAnimTime + 0.2, mat4.create()));
    this.graph.animations["button" + clickedObject.id] =
  ...
  genDeadPieceAnimation(checker) {
    ...
    let finalPosition = vec3.fromValues(
      finalMatrix[12],
      finalMatrix[13],
      finalMatrix[14]
    );
  ...
  move(checker, tile) {
    ...
    checkerPickable.children = checkerPickable.children.filter(
      (child) => child.id !== checkerObject.id
    );
    ...
    if (Math.abs(checker.row - tile.row) === 2) {
      ...
      this.board[jumpedChecker.row][jumpedChecker.column] = new Tile(
        jumpedChecker.row,
        jumpedChecker.column,
        jumpedCheckerPickable
      );
      checker.addAnimation(this.genAnimation(checker, tile, true));

      this.gameInfo[this.currentPlayer].taken++;
      if (jumpedChecker.isQueen())
        this.gameInfo[this.currentPlayer === "black" ? "white" : "black"]
          .queens--;
    } else {
  ...
  verifyGameInfo() {
    ...
    if (blackTaken !== this.gameInfo.black.taken)
      this.gameInfo.black.taken = blackTaken;
    if (whiteTaken !== this.gameInfo.white.taken)
      this.gameInfo.white.taken = whiteTaken;
    if (blackQueens !== this.gameInfo.black.queens)
      this.gameInfo.black.queens = blackQueens;
    if (whiteQueens !== this.gameInfo.white.queens)
      this.gameInfo.white.queens = whiteQueens;
  }
    ...
    if (this.state === states.canLock || this.state === states.onCombo) {
      ...
      for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
          const tile = this.board[i][j];
          if (!tile) continue;
          if (tile.isChecker()) tile.deselect();
  ...
  async getGameMovie() {
    ...
    for (const move of gameMoves) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.animTime * 1000 + 300)
      );
  ...
  play(selected) {
    ...
    if (this.state === states.playing) {
      if (
        selectedTile.isChecker() &&
        selectedTile.color === this.currentPlayer
      ) {
      ...
      if (
        ...
        if (moves.moves.includes(selectedTile)) {
          ...
          this.selectedTile.deselect();
          this.currBoard = this.board.map((row) =>
            row?.map((tile) => tile?.clone())
          );
